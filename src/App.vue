<template>
  <div id="app">
    ...
    <!-- <canvas id="canvas_1"
      ...
      height="500"></canvas> -->
    <!-- <div class="Image">
      <img :src="require(`@/image/${imgIndex}.jpg`)"
        alt="">
    </div> -->
    <div>this is a {{this.message}}</div>
    <div>this is a {{reversedMessage}}</div>
    <div>this is a {{fullName}}</div>
    <div :key="item.id"
      v-for="item in items">{{item.message}}</div>
    <div id="todo">
      <input style="width:100px;height:20px"
        @keyup.enter="intoTodoList"
        v-model="todo">
      <ul>
...
export default {
  ...
  data() {
    return {
      ...
      todo: '',
      todoList: ['Do the dishes', 'Take out the trash', 'Mow the lawn'],
    }
  },
  computed: {
    reversedMessage() {
